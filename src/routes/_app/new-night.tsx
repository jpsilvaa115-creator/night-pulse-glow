...
import { DRINK_PRESETS, type Drink } from "@/lib/destrava-store";
import { createNight } from "@/lib/nights-api";
...
function NewNight() {
  ...
  const finish = async () => {
    ...
    toast.success("Noite registrada!");
  ...
  return (
    <div className="container mx-auto max-w-2xl px-4 sm:px-6 py-6 space-y-5">
      ...
      <section className="glass rounded-3xl p-6 space-y-4">
        ...
        <Field label="Foto da noite (opcional)">
          <label className="flex items-center justify-center gap-2 px-4 py-6 rounded-xl border-2 border-dashed border-border bg-secondary/40 cursor-pointer hover:border-primary transition-colors">
            ...
            <span className="text-sm">{photoPreview ? "Foto adicionada — trocar" : "Adicionar foto"}</span>
            <input type="file" accept="image/*" onChange={onPhoto} className="hidden" />
          </label>
          {photoPreview && <img src={photoPreview} alt="" className="mt-3 rounded-xl w-full max-h-64 object-cover" />}
        </Field>
      </section>
      ...
      <button onClick={finish} disabled={drinks.length === 0 || saving}
        ...
        {saving ? "Salvando…" : "Finalizar e ver resumo"}
      </button>
    </div>
  );
}
